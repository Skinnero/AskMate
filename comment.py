...
def route_comment_add_to_question(question_id):
...
def route_comment_add_to_answer(answer_id):
    if request.method == "GET":
...
def route_comment_delete(comment_id):
    comment = read_single_row_from_db_by_id(COMMENT,comment_id)
    id = read_single_row_from_db_by_id(ANSWER,comment['answer_id'])['question_id'] if comment.get('question_id') == None else comment['question_id']
